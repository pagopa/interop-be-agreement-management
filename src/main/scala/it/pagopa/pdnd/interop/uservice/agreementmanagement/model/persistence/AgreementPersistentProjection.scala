...
class AgreementPersistentProjection(system: ActorSystem[_], entity: Entity[Command, ShardingEnvelope[Command]]) {
  ...
  def projection(tag: String): AtLeastOnceFlowProjection[Offset, EventEnvelope[Event]] = {
    CassandraProjection
      .atLeastOnceFlow(projectionId = ProjectionId("agreement-projections", tag), sourceProvider(tag), handler = flow)
  ...
  val projections: Seq[AtLeastOnceFlowProjection[Offset, EventEnvelope[Event]]] =
    (0 until settings.numberOfShards).map(i =>
      projection(s"pdnd-interop-uservice-agreement-management-persistence-agreement|$i")
    )
