package it.pagopa.pdnd.interop.uservice.agreementmanagement.server
...
class Controller(agreement: AgreementApi, validationExceptionToRoute: Option[ValidationException => Route] = None)(implicit system: ActorSystem) {
  ...
  */
  def getBuildInfo: Route =
    path("pdnd-interop-uservice-agreement-management" / "build-info") {
      get {
        complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
      }
   }

  lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-agreement-management" / interfaceVersion) {
